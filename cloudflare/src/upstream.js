...
import { extractCookieValue, generateNonce, getAcSignature } from './signature.js';
...
async function requestText(url, env, options = {}) {
...
async function bootstrapGuestCookie(env, options = {}) {
  const forceRefresh = Boolean(options.forceRefresh);
  const now = Date.now();
  if (!forceRefresh && guestCookieCache?.cookies && now - guestCookieCache.startTimestamp < GUEST_COOKIE_TTL_MS) {
    return guestCookieCache.cookies;
  }

  const { response } = await requestText(LIVE_HOME_URL, env, {
    method: 'GET',
    proxy: resolveProxyValue(options, env),
    headers: {
      'Accept-Language': 'zh-CN,zh;q=0.9,en;q=0.8',
  ...
  const cookies = toCookieHeader(getSetCookieHeaders(response.headers));
  if (!cookies || !cookies.includes('ttwid=')) {
    if (guestCookieCache?.cookies) return guestCookieCache.cookies;
    throw new Error('Failed to bootstrap guest cookie because upstream did not return ttwid');
  }

  guestCookieCache = { startTimestamp: now, cookies };
  return cookies;
}

async function getHtmlNonce(url, env, options = {}) {
  const forceRefresh = Boolean(options.forceRefresh);
  const now = Date.now();
  if (!forceRefresh && htmlNonceCache?.nonce && now - htmlNonceCache.startTimestamp < HTML_NONCE_TTL_MS) {
    return htmlNonceCache.nonce;
  }

  const { response } = await requestText(url, env, {
    method: 'GET',
    proxy: resolveProxyValue(options, env),
    headers: {
      'Accept-Language': 'zh-CN,zh;q=0.9,en;q=0.8',
  ...
  htmlNonceCache = { startTimestamp: now, nonce: finalNonce };
  return finalNonce;
}
...
function toWebLikePayload({ room, user, enterRoomId, roomStatus, enterMode, qrcodeUrl, partitionRoadMap, isLogin, serverTimeMs }) {
  return {
    status_code: 0,
    data: {
      data: [room],
      ...
      login_lead: { is_login: Boolean(isLogin) },
    },
    extra: { now: serverTimeMs ?? Date.now() },
  };
}

function buildHtmlFallbackPayload(stateData, isLogin) {
...
function buildMobileFallbackPayload(room) {
  const synthesizedRoom = {
    ...
    linker_detail: room?.linker_detail || { linker_ui_layout: room?.linkmic_layout ?? 0 },
    live_room_mode: room?.live_type_audio ? 1 : 0,
    room_cart: room?.room_cart || { contain_cart: false, total: 0 },
    paid_live_data: room?.paid_live_data || { paid_type: 0, view_right: 0 },
  };
...
async function requestRoomEnter(url, webRid, cookie, userAgent, env, options = {}) {
  return requestText(url, env, {
    method: 'GET',
    proxy: resolveProxyValue(options, env),
...
async function fetchRoomInfoByWeb(webRid, env, options = {}) {
  const aid = options.aid || DEFAULT_AID;
  const explicitCookie = options.cookie || env.DOUYIN_COOKIE || '';
  const requestConfigs = [buildRoomEnterRequest(webRid, aid), buildPlainRoomEnterRequest(webRid, aid)];
  const errors = [];

  for (const requestConfig of requestConfigs) {
    const attempts = [];
    if (explicitCookie) attempts.push({ label: 'provided cookie', cookie: explicitCookie, canRefresh: false });
    attempts.push({ label: 'guest cookie', cookie: await bootstrapGuestCookie(env, options), canRefresh: true });
    ...
    for (const attempt of attempts) {
      ...
      for (let retry = 0; retry < 2; retry += 1) {
        if (!cookie || triedCookies.has(cookie)) break;
        triedCookies.add(cookie);

        try {
          const { response, text } = await requestRoomEnter(requestConfig.url, webRid, cookie, requestConfig.userAgent, env, options);
          if (!response.ok) {
          ...
          return {
            payload,
            upstream: { status: response.status, aid, url: requestConfig.url },
          };
        } catch (error) {
          errors.push(`${requestConfig.label}/${attempt.label}: ${error.message}`);
          if (!attempt.canRefresh || retry > 0) break;
          cookie = await bootstrapGuestCookie(env, { ...options, forceRefresh: true });
        }
      }
    }
  }
...
async function fetchRoomInfoByHtml(webRid, env, options = {}) {
  const aid = options.aid || DEFAULT_AID;
  ...
  if (!cookie) {
    const nonce = await getHtmlNonce(url, env, options);
    cookie = buildHtmlSignatureCookie(url, nonce, DEFAULT_USER_AGENT);
  }

  const { response, text } = await requestText(url, env, {
    method: 'GET',
    proxy: resolveProxyValue(options, env),
    headers: buildHeaders(webRid, cookie, DEFAULT_USER_AGENT),
  });

  if (!response.ok) throw new Error(`Room HTML request failed with status ${response.status}. Body preview: ${text.slice(0, 200)}`);
  if (text.includes('验证码')) throw new Error('Room HTML fallback hit verification challenge');

  const match = text.match(ROOM_HTML_STATE_REGEX);
  if (!match) throw new Error('Room HTML fallback could not find the embedded state payload');

  const jsonText = match[1].replace(/\\"/g, '"').replace(/\\"/g, '"');
  const stateData = JSON.parse(jsonText);
  const result = buildHtmlFallbackPayload(stateData, isLogin);

  return {
    payload: result.payload,
    upstream: { status: response.status, aid, url },
...
async function fetchRoomInfoByMobile(webRid, secUid, env, options = {}) {
  ...
  const { response, text } = await requestText(url.toString(), env, {
    method: 'GET',
    proxy: resolveProxyValue(options, env),
    headers: {
      'Accept-Language': 'zh-CN,zh;q=0.9,en;q=0.8',
  ...
  if (!response.ok) throw new Error(`Mobile fallback request failed with status ${response.status}. Body preview: ${text.slice(0, 200)}`);
  const payload = parsePayload(text, response);
  const room = payload?.data?.room;
  if (!room) throw new Error('Mobile fallback payload did not contain room data');

  return {
    payload: buildMobileFallbackPayload(room),
    upstream: { status: response.status, aid, url: url.toString() },
  };
}

async function enrichTimeFromMobile(webRid, secUid, basePayload, env, options = {}) {
  if (!secUid) return basePayload;
  try {
    const mobileResult = await fetchRoomInfoByMobile(webRid, secUid, env, options);
    const mobileRoom = mobileResult?.payload?.data?.data?.[0];
    const baseRoom = basePayload?.data?.data?.[0];
    if (mobileRoom && baseRoom) {
      if (mobileRoom.create_time != null && baseRoom.create_time == null) baseRoom.create_time = mobileRoom.create_time;
      if (mobileRoom.start_time != null && baseRoom.start_time == null) baseRoom.start_time = mobileRoom.start_time;
      if (mobileRoom.finish_time != null && baseRoom.finish_time == null) baseRoom.finish_time = mobileRoom.finish_time;
    }
  } catch {
    // best effort only
  }
...
export async function fetchRoomEnter(webRid, env, options = {}) {
  if (!webRid) throw new Error('webRid is required');
  ...
  try {
    const webResult = await fetchRoomInfoByWeb(webRid, env, options);
    const webSecUid = webResult?.payload?.data?.user?.sec_uid || '';
    webResult.payload = await enrichTimeFromMobile(webRid, secUid || webSecUid, webResult.payload, env, options);
    return webResult;
  } catch (error) {
  ...
  if (secUid) {
    try {
      return await fetchRoomInfoByMobile(webRid, secUid, env, options);
    } catch (error) {
  ...
  try {
    htmlResult = await fetchRoomInfoByHtml(webRid, env, { ...options, cookie: explicitCookie });
    secUid = secUid || htmlResult.secUid;
    if (htmlResult?.payload?.data?.data?.[0]?.stream_url) {
      htmlResult.payload = await enrichTimeFromMobile(webRid, secUid, htmlResult.payload, env, options);
      return htmlResult;
    }
  } catch (error) {
