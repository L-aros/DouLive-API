...
class CryptoUtility {
  ...
  paramsToArray(value, addSalt) {
    return CryptoUtility.sm3ToArray(addSalt ? this.addSalt(value) : value);
  }
  ...
  transformBytes(values) {
    ...
    for (let index = 0; index < values.length; index += 1) {
      ...
      [this.bigArray[newSumIndex], this.bigArray[nextIndex]] = [this.bigArray[nextIndex], this.bigArray[newSumIndex]];
      indexB = newSumIndex;
    }
...
export class ABogus {
  constructor(browserFingerprint, userAgent, options) {
    ...
    this.userAgent = userAgent && userAgent.length > 0 ? userAgent : 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/130.0.0.0 Safari/537.36 Edg/130.0.0.0';
    this.browserFingerprint = browserFingerprint && browserFingerprint.length > 0 ? browserFingerprint : BrowserFingerprintGenerator.generateFingerprint('Edge');
    this.options = options || [0, 1, 14];
    this.pageId = 0;
    this.aid = 6383;
    this.uaKey = [0x00, 0x01, 0x0e];
    this.sortIndex = [18,20,52,26,30,34,58,38,40,53,42,21,27,54,55,31,35,57,39,41,43,22,28,32,60,36,23,29,33,37,44,45,59,46,47,48,49,50,24,25,65,66,70,71];
    this.sortIndex2 = [18,20,26,30,34,38,40,42,21,27,31,35,39,41,43,22,28,32,36,23,29,33,37,44,45,46,47,48,49,50,24,25,52,53,54,55,57,58,59,60,65,66,70,71];
  }
  ...
  generateAbogus(params, body) {
    ...
    const randomPrefix = StringProcessor.generateRandomBytes(3).split('').map((char) => char.charCodeAt(0));
