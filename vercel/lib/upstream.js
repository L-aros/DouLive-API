...
async function getHtmlNonce(url, options = {}) {
  const forceRefresh = Boolean(options.forceRefresh);
  const now = Date.now();
...
async function fetchRoomInfoByWeb(webRid, options = {}) {
  ...
  for (const requestConfig of requestConfigs) {
    const attempts = [];
    if (explicitCookie) {
      attempts.push({ label: 'provided cookie', cookie: explicitCookie, canRefresh: false });
    }
    attempts.push({ label: 'guest cookie', cookie: await bootstrapGuestCookie(options), canRefresh: true });
    ...
    for (const attempt of attempts) {
      ...
      for (let retry = 0; retry < 2; retry += 1) {
        ...
        try {
          ...
          cookie = await bootstrapGuestCookie({ ...options, forceRefresh: true });
        }
      }
    }
  }
...
async function fetchRoomInfoByHtml(webRid, options = {}) {
  ...
  if (!cookie) {
    const nonce = await getHtmlNonce(url, options);
...
async function fetchRoomEnter(webRid, options = {}) {
  ...
  try {
    htmlResult = await fetchRoomInfoByHtml(webRid, { ...options, cookie: explicitCookie });
